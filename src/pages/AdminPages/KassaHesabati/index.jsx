...
const KassaHesabati = () => {
    ...
    const filteredReporte = reporte?.filter((row) => {
        ...
        return row.categoryName.toLowerCase().includes(searchName.toLowerCase());
    });
    ...
    const PRODUCT_BY_CATEGORY = selectedCat
        ? CATEGORY_LIST.find((c) => c.id === selectedCat)?.products || []
        : [];
    ...
    const addProduct = (id) => {
        if (id !== "__all__" && !selectedProds.includes(id)) {
            setSelectedProds([...selectedProds, id]);
        }
        setProdVal("__all__");
        setIsProdOpen(false);
    };
    ...
    const { data: chartData, isFetching } = useGetByIdCashReporteChartQuery({
        companyId: id,
    ...
    return (
        <div className="admin-kassa-h-main">
            <div className="admin-kassa-h">
                ...
                <div className="below-table-filters">
                    <div className="dropdown-row">
                        <div className="custom-select" ref={catRef}>
                            ...
                            >
                                <span>{selectedCat ? CATEGORY_LIST.find((c) => c.id === selectedCat)?.name : "Kateqoriya seç"}</span>
                                <svg
                                    className={`chev ${isCatOpen ? "rotate" : ""}`}
                                    width="16"
                                    height="16"
                                    viewBox="0 0 24 24"
                                >
                    ...
                    <div className="chips">
                        {selectedCat && (
                            <span className="chip">
                {CATEGORY_LIST.find((c) => c.id === selectedCat)?.name}
                                <button className="chip__x" onClick={removeCat}>
                  <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="16"
                  ...
                  >
                    ...
                    <div className="dropdown-row mtop">
                        <div className="custom-select" ref={prodRef}>
                            <div
                                className="select-trigger"
                                onClick={(e) => {
                                    if (selectedCat) {
                                ...
                                style={selectedCat ? {} : { opacity: 0.5, cursor: "not-allowed" }}
                            >
                                <span>{selectedProds.length > 0 ? "Məhsul seçildi" : "Məhsul seç"}</span>
                                <svg
                                    className={`chev ${isProdOpen ? "rotate" : ""}`}
                                    width="16"
                                    height="16"
                                    viewBox="0 0 24 24"
                                >
                            ...
                            {isProdOpen && selectedCat && (
                                <ul className="options">
                                    {PRODUCT_BY_CATEGORY.map((p) => (
                                        <li
                                            key={p.id}
                                            onClick={() => addProduct(p.id)}
                                            className={selectedProds.includes(p.id) ? "active" : ""}
                                        >
                                            {p.name}
                                        </li>
                                    ))}
                    ...
                    <div className="chips">
                        {selectedProds.map((pid) => {
                            const prod = PRODUCT_BY_CATEGORY.find((p) => p.id === pid);
                            return (
                                <span className="chip" key={pid}>
                  {prod?.name}
                                    <button className="chip__x" onClick={() => removeProd(pid)}>
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                        height="16"
                ...
                {/* ===== Chart ===== */}
                {selectedProds.length > 0 && chartData?.data && (
                    <div className="card chart-card">
                            ...
                            <ResponsiveContainer width="100%" height={240}>
                                ...
                                >
                                    ...
                                    <Bar
                                        ...
                                        radius={[6, 6, 0, 0]}
                                        barSize={chartData?.data?.length === 1 ? 60 : undefined}
                                    >
                        ...
                        <div className="chart-card__legend">
                            {chartData.data.map((d, i) => (
                                <div className="legend-item" key={d.productId}>
                  <span
                      className="dot"
                      style={{ background: PROD_COLORS[i % PROD_COLORS.length] }}
                  />
                                    <span className="name">{d.productName}</span>
                                    <span className="count">({d.operationCount})</span>
                                </div>
                            ))}
                        </div>
                    </div>
