...
const SuperAdminBolme = () => {
    ...
    const pageSize = 5;
    const [editSectionData, setEditSectionData] = useState({ id: '', name: '' });

    const [searchName, setSearchName] = useState('');
    const [activeSearch, setActiveSearch] = useState(null);
    const {data: getSobeBYDepartmentId,refetch} =useGetSobeBYDepartmentIdQuery(id)
    ...
    const filteredSections = data?.filter(item =>
        item.name.toLowerCase().includes(searchName.toLowerCase())
    );
    const totalPages = Math.ceil(filteredSections?.length / pageSize);
    const pagedSections = filteredSections?.slice((currentPage - 1) * pageSize, currentPage * pageSize);
    ...
    return (
        <div className="super-admin-bolme-main">
            <div className="super-admin-bolme">
                <div className={"root"}>
                    <h2 >
                ...
                <div className={"paths"}>
                    ...
                    <div className={'path2'}>
                        <h3>Şöbə adı:</h3> <span>{department?.name}</span>
                    </div>
                </div>
                <div className="order-table-wrapper">
                        <table>
                            ...
                            {pagedSections?.map((section, idx) => (
                                <tr key={section.id}>
                                    <td>{section.name}</td>
                                    <td>{section.customers?.length || 0}</td>
                                    <td>
                                        <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
                                            <svg style={{cursor:"pointer"}}  onClick={() => {
                                                setEditSectionData({ id: section.id, name: section.name });
                                            ...
                                            <svg style={{cursor:"pointer"}} onClick={() => setDeleteSectionId(section.id)} xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                ...
                <div className="super-admin-bolme__pagination">
                    ...
                    <div className="vendor-edit-modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-fields">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setEditSectionData((prev) => ({ ...prev, name: e.target.value }))
                                }
                            />
                        ...
                        <button
                            className="save-btn"
                            onClick={async () => {
                                if (!editSectionData.name.trim()) return;

                                try {
                                    await edit({
                                        id: editSectionData.id,
                                        name: editSectionData.name,
                                        departmentId: id, // params'dan gəlir
                                    });
                ...
                <div className="modal-overlay" onClick={() => setDeleteSectionId(null)}>
                    <div className="delete-modal-box" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="delete-modal-actions">
                            <button className="cancel-btn" onClick={() => setDeleteSectionId(null)}>Ləğv et</button>
                            <button
...
export default SuperAdminBolme;
