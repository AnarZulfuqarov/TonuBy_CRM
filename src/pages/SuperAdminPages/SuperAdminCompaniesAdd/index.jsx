...
const SuperAdminCompanyAdd = () => {
    ...
    const handleSubmit = async () => {
        try {
            for (const row of rows) {
                if (row.name.trim() !== '') {
                    const response = await createCompany({ name: row.name });
                    if (!('data' in response)) {
                        throw new Error('Bir və ya bir neçə şirkət əlavə edilə bilmədi.');
                    }
                }
            }
            setRows([{ name: '' }]);
    ...
    return (
        <div className="super-admin-company-add-main">
            <div className="super-admin-company-add">
                <div className="headerr">
                    <div className="head">
                        <h1>Şirkət əlavə edilməsi</h1>
                    </div>
                    <h2>
                        <NavLink className="link" to="/superAdmin/companies">— Şirkət</NavLink> — Şirkət əlavə edilməsi
                    </h2>
                </div>
            ...
            {showSuccessModal && (
                <div className="modal-overlay" onClick={() => setShowSuccessModal(false)}>
                    <div className="success-modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <button className="back-btn" onClick={() => window.location.href = "/superAdmin/companies"}>
                            Əsas səhifəyə qayıt
                        </button>
                    </div>
                </div>
